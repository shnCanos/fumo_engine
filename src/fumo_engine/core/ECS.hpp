...
public:
  ...
  void serialize_component(const EntityId &entity_id, ComponentId component_id,
                           cereal::JSONOutputArchive &out_archive) {
    component_manager->serialize_component(entity_id, component_id,
                                           out_archive);
  }
  ...
  template <typename T> [[nodiscard]] T &get_component(EntityId entity_id) {
  ...
  template <typename T> void check_for_component(EntityId entity_id) {
    return component_manager->check_for_component<T>(entity_id);
  }

  template <typename T> [[nodiscard]] ComponentId get_component_id() {
