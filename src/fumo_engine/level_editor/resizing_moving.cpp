...
void DebugLevelEditor::move_entity(FumoVec2 mouse_position) {
    for (const auto& entity_id : sys_entities) {
        ...
        EntityQuery circle_grav_query {
            .component_mask =
                fumo_engine->ECS->make_component_mask<CircularGravityField>(),
        ...
        if (distance < mouse_radius) {
            ...
            if (fumo_engine->ECS->filter(entity_id, circle_grav_query)) {
                auto& gravity_field =
                    fumo_engine->ECS->get_component<CircularGravityField>(
                        entity_id);
                gravity_field.position = mouse_position;

            } else if (fumo_engine->ECS->filter(entity_id, only_grav_query)) {
            ...
            } else if (fumo_engine->ECS->filter(entity_id,
                                                parallel_grav_query)) {
                auto& parallel_field =
                    fumo_engine->ECS->get_component<ParallelGravityField>(
                        entity_id);
...
void DebugLevelEditor::drag_resizing(FumoVec2 mouse_position) {
    ...
    for (const auto& entity_id : sys_entities) {
        ...
        if (fumo_engine->ECS->filter(entity_id, parallel_grav_query)) {
            auto& parallel_field =
                fumo_engine->ECS->get_component<ParallelGravityField>(
                    entity_id);
            //
